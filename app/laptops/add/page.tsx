...
export default function AddLaptopPage() {
  ...
  useEffect(() => {
    ...
    const pickToken = (t: string | null) => !!t && t !== "undefined" && t !== "null" && t.trim() !== ""
    const isTokenValid = pickToken(idToken) || pickToken(accessToken)
    if (isTokenValid) {
      setIsLoggedIn(true)
    } else {
      setIsLoggedIn(false)
      ...
      router.push("/login?redirect=/laptops/add")
    }
  }, [router, toast])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const accessToken = localStorage.getItem("accessToken")
      const pickToken = (t: string | null) => !!t && t !== "undefined" && t !== "null" && t.trim() !== ""
      const tokenToUse = (pickToken(idToken) ? idToken! : accessToken!).trim()
      if (!pickToken(tokenToUse)) {
        throw new Error("Authentication required")
      }
      console.log("Submitting laptop data:", formData)

      // Call the API to add a new laptop via centralized client
      const data = await addLaptop({
        brand: formData.brand,
        model: formData.model,
        price_per_day: Number(formData.price_per_day),
      }, tokenToUse)
      console.log("Add laptop response:", data)
      ...
      if (
        ...
        error.message.includes("Unauthorized") ||
        error.message.includes("Invalid or missing authentication token") ||
